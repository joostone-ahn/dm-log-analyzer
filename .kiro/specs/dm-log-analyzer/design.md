...
### v1.2.2 (2026-01-29) - Docker 환경 NR RRC 파싱 문제 해결

#### 14.1 문제 정의
**증상**: Docker 환경에서 NR RRC 메시지가 파싱되지 않음
- 로컬 환경: 정상 파싱 (302개 메시지)
- Docker 환경: NR RRC 누락 (254개 메시지)
- scat Lua 플러그인 로드 실패로 인한 문제

**원인 분석**:
```bash
# Docker 컨테이너 내부에서 tshark 실행 시
tshark -X lua_script:/tmp/scat.lua -r pcaps/VoNR.pcap -T json
# 에러: Lua 플러그인 로드 실패 (stderr에 출력)
# 결과: nr-rrc 레이어가 JSON에 포함되지 않음
```

- Lua 플러그인이 임시 디렉토리에서 로드되지 않음
- tshark가 Lua 플러그인을 찾지 못해 NR RRC dissector 비활성화
- 로컬 환경에서는 전역 플러그인 디렉토리에 설치되어 정상 동작
...
COPY wireshark/scat.lua /usr/lib/x86_64-linux-gnu/wireshark/plugins/
```

**2단계: converters.py 수정**
```python
# 임시 디렉토리 대신 전역 플러그인 사용
lua_plugin_path = '/usr/lib/x86_64-linux-gnu/wireshark/plugins/scat.lua'

if not os.path.exists(lua_plugin_path):
    # 로컬 환경 폴백
    lua_plugin_path = os.path.join(os.path.dirname(__file__), 'wireshark', 'scat.lua')

# tshark 명령어에서 -X lua_script 옵션 제거
cmd = [
    'tshark',
    '-r', pcap_path,
    '-T', 'json'
...
**3단계: docker-compose.yml 개선**
```yaml
services:
  dm-log-analyzer:
    image: ghcr.io/your-username/dm-log-analyzer:latest
    container_name: dm-log-analyzer
...
$ docker compose up -d
$ # VoNR.pcap 업로드
$ # 결과: 302개 메시지 파싱 (로컬과 동일)
```

**플랫폼 호환성**:
- AMD64: 정상 동작 (Windows, Linux, macOS Intel)
- ARM64: Rosetta 2 에뮬레이션으로 정상 동작 (macOS Apple Silicon)

#### 14.4 영향 범위

**긍정적 영향**:
- Docker 환경에서 NR RRC 메시지 정상 파싱
- 로컬 환경과 동일한 파싱 결과
- 크로스 플랫폼 일관성 확보

**호환성**:
- 로컬 환경: 기존 방식 유지 (하위 호환성)
- Docker 환경: 전역 플러그인 사용
- 모든 플랫폼에서 동일한 결과
...
- [x] docker-compose.yml: version 제거, platform 명시
- [x] Dockerfile: Lua 플러그인 전역 설치
- [x] converters.py: Lua 플러그인 경로 수정

---

### v1.2.1 (2026-01-28) - GitHub Container Registry 지원

#### 14.6 개선 사항
- GitHub Container Registry 자동 빌드 지원
- README 간소화 및 Docker 배포 가이드 분리
- .dockerignore 최적화
